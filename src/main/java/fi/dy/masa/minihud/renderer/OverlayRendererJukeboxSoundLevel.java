...
public class OverlayRendererJukeboxSoundLevel extends OverlayRendererBase {
    public static final OverlayRendererJukeboxSoundLevel INSTANCE = new OverlayRendererJukeboxSoundLevel();
    ...
    private boolean updateLightLevels(World world, BlockPos center) {
        ...
        int radius = Configs.Generic.JUKEBOX_SOUND_LEVEL_RANGE.getIntegerValue();
    ...
    private int distanceToNearestJukebox(BlockPos pos) {
        int minDist = 0;
        for (BlockPos jukeboxPos : MiniHUD.jukeboxes) {
            if (jukeboxPos.isWithinDistance(pos, 65 - 2)) {
                int dist = (int) jukeboxPos.getSquaredDistance(pos);
